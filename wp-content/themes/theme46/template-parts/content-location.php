<?php 
   ...
   Template for displaing content of single location in single-location.php file
   
   */
   ?>
<?php
   global $location_banner_title,$location_phone,$location_fax,$location_email,$location_address,$location_address_url,$location_hours,$enable_disable_location_social_links,$location_social_links,$phone_title,$address_title,$email_title,$fax_title;
   $location_phone = get_field('phone_number');
   $location_fax = get_field('fax');
   $location_email = get_field('email_id');
   $location_address = get_field('address');
   $location_address_url = get_field('address_url');
   ...
   $location_hours = get_field('location_working_hours');
   $enable_disable_location_social_links=get_field('enable_disable_location_social_links');
   $location_social_links = get_field('location_social_links'); 
   ?>

<?php if(have_rows('homepage_content')):?>
<?php while(have_rows('homepage_content')):the_row();?>
<?php if(get_row_layout() == 'location_contact_hours_gallery'):?>
<section class="locdetailinfo-section">
   <div class="container">
   <div class="row locdetailinfo-row">
   <?php if((get_sub_field('use_working_hours')) || (get_sub_field('use_location_contact_details'))): ?>
   <?php if(get_sub_field('use_location_gallery')): ?>
   <div class="col-lg-8 col-sm-12">
   <?php else:?>
   <div class="col-lg-12 col-sm-12 fullcontact-locinfo">
   <?php endif;?>
   <div class="row">
      <?php if(get_sub_field('use_location_contact_details')): ?>
      <?php if(get_sub_field('use_working_hours')): ?>
      <div class="col-md-6 col-sm-6 locontactinfo-col">
         <?php else:?>
         <div class="col-md-12 col-sm-12 locontactinfo-col">
            <?php endif;?>
            <div class="locontact-detail">
               <?php if(get_field('title')):?>
               <div class="inner-title">
                  <?php if(empty(get_field('page_title'))): ?>
               ...
               <?php endif;?>
               <div class="locontact-info footcontact-info">
                  <?php if($location_address):?>
                  <div class="footinfo-text foot-adderss">
                     <a href="<?php echo $location_address_url; ?>" target="_blank"><i class="fas fa-map-marker-alt"></i><?php echo $location_address; ?></a>
                  </div>
                  <?php endif;?>
                  <?php if($location_email):?>
                  <div class="footinfo-text foot-mail">
                     <a href="mailto:<?php echo $location_email; ?>"><i class="fa fa-envelope"></i><?php echo $location_email; ?></a>
                  </div>
                  <?php endif;?>
                  <?php if($location_phone):?>
                  <div class="footinfo-text foot-phone">
                     <a href="tel:<?php echo $location_phone; ?>"><i class="fas fa-phone"></i><?php echo $location_phone; ?></a>
                  </div>
                  <?php endif;?>
                  <?php if($location_fax):?>
                  <div class="footinfo-text foot-fax">
                     <span><i class="fas fa-fax"></i><?php echo $location_fax; ?></span>
                  </div>
                  <?php endif;?>
               </div>
               <?php if(($enable_disable_location_social_links) && ($location_social_links)):?>
               <div class="foot-social location-social">
                  <ul class="socialmedia">
                     <?php foreach ($location_social_links as $soc_val):?>
                     <li><a href="<?php echo $soc_val['social_link_url'];?>" target="_blank" title="<?php echo $soc_val['social_title']; ?>"><i class="<?php echo $soc_val['social_icon_class'];?>"></i></a></li>
                     <?php endforeach;?>
                  </ul>
               </div>
               <?php endif;?>
         ...
         <?php endif;?>
         <?php if(get_sub_field('use_working_hours')):?>
         <?php $location_working_hours=$location_hours['working_hours']; ?>
         <?php if($location_working_hours): ?>
         <?php if(get_sub_field('use_location_contact_details')):?>
         <div class="col-md-6 col-sm-6 lochours-col">
            <?php else:?>
            <div class="col-md-12 col-sm-12 lochours-col full-lochours">
               <?php endif;?>
               <?php if($location_hours['working_hours_title']): ?>
               <div class="inner-title">
                  <h2><?php echo $location_hours['working_hours_title'];?></h2>
               </div>
               <?php endif;?>
               <div class="foothours locdetail-hours">
                  <ul class="foothourslist clearfix">
                     <?php foreach ($location_working_hours as $hr_val):?>
                     <li>
                        <span class="footday"><?php echo $hr_val['day']; ?></span>
                        <span class="foottime"><?php echo $hr_val['opening_time']; ?> <?php if($hr_val['closing_time']): ?> - <?php echo $hr_val['closing_time']; ?> <?php endif;?></span>
                     </li>
                     <?php endforeach;?>
                     <?php if($location_hours['working_hours_note']): ?>
                     <li class="hoursnote">
                        <?php echo $location_hours['working_hours_note'];?>
                     </li>
                     <?php endif;?>
                  </ul>
               </div>
      ...
      <?php if($location_gallery):?>
      <?php if((get_sub_field('use_working_hours')) || (get_sub_field('use_location_contact_details'))): ?>
      <div class="col-lg-4 col-sm-12 locgallery-col">
         <?php else:?>
         <div class="col-lg-12 col-sm-12 locgallery-col">
            <?php endif;?>
            <?php $location_gallery=get_sub_field('location_gallery'); ?>
            <?php if($location_gallery):?>
            <div class="row locgallerylist mfpgallery locgalleryslider owl-carousel">
               <?php foreach ($location_gallery as $locgal):?>
               <div class="col-lg-4 col-sm-6 item">
                  <div class="locgallery-box">
                     <a class="locgallery-thumb" href="<?php echo $locgal['url'];?>">
                     <img src="<?php echo $locgal['sizes']['lol-gallery-thumb']; ?>" alt="<?php $locgal['alt']; ?> " width="416" height="360" />
                     </a>
                  </div>
               </div>
               <?php endforeach;?>
