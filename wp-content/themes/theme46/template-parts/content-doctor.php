<?php 
   /*
   Template for displaing content of single doctor in single-doctor.php file
   */
   ?>
<?php $style=$backcolor=$backimg='';?>
<?php if(get_field('doctor_details_page_background_color','option')):?>
<?php $backcolor = 'background-color:'.get_field('doctor_details_page_background_color','option').';';?>
<?php endif; ?>
<?php if(get_field('doctor_details_page_background_image','option')):?>
<?php $backimg = 'background-image:url('.get_field('doctor_details_page_background_image','option')['url'].');';?>
<?php endif; ?>
<?php if((get_field('doctor_details_page_background_color','option')) || (get_field('doctor_details_page_background_image','option'))): ?>
...
<?php if(get_field('enable_other_doctors','option')):?>
<?php $style=$backcolor=$backimg='';?>
<?php if(get_field('other_doctors_background_color','option')):?>
<?php $backcolor = 'background-color:'.get_field('other_doctors_background_color','option').';';?>
<?php endif; ?>
<?php if(get_field('other_doctors_background_image','option')):?>
<?php $backimg = 'background-image:url('.get_field('other_doctors_background_image','option')['url'].');';?>
<?php endif; ?>
<?php if((get_field('other_doctors_background_color','option')) || (get_field('other_doctors_background_image','option'))): ?>
<?php $style='style='.'"'.$backcolor.$backimg.'"'; ?>
<?php endif;?>
<section class="otherdoctor-section" <?php echo $style;?>>
   <div class="container">
      <?php if((get_field('other_doctor_title','option')) || (get_field('other_doctor_sub_title','option'))):?>
      <div class="maintitle">
         <?php if(get_field('other_doctor_title','option')):?>
         <h1 data-aos="fade-down" data-aos-duration="2000"><?php echo get_field('other_doctor_title','option'); ?></h1>
         <?php endif;?>
         <?php if(get_field('other_doctor_sub_title','option')):?>
         <p data-aos="fade-up" data-aos-duration="2000"><?php echo get_field('other_doctor_sub_title','option'); ?></p>
         <?php endif;?>
      </div>
      <?php endif;?>
      <div class="row doctor-list othrdoctor-slider owl-carousel">
         <?php 
            $select_other_doctor=get_field('select_other_doctor','option'); 
            $other_doctors_read_more_button_text=get_field('other_doctors_read_more_button_text','option');
            $currentid=get_the_ID();
            ?>
         <?php if($select_other_doctor):?>
         <?php foreach ($select_other_doctor as $team_val) :?>
         ...
         <div class="col-lg-4 col-sm-6 item" data-aos="zoom-in" data-aos-duration="2000">
            <div class="drlist-box">
               ...
               <?php if(($other_doctors_read_more_button_text) && (get_field('enable_details',$team_val->ID))):?>
               <div class="drlistbox-btn">
                  <a class="gradient-btn" href="<?php echo get_the_permalink($team_val->ID); ?>"><?php echo $other_doctors_read_more_button_text; ?></a>
               </div>
               <?php endif;?>
            </div>
         </div>
